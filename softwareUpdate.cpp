...
void softwareUpdate::on_pushButton_update_clicked()
{
    ...
    else if(ret == QMessageBox::Yes)
    {
        ...
        messageBox.button(QMessageBox::Cancel)->setText("取消");
        QString log;
        log = "确定U盘/root文件夹下包含程序文件(Monitor、libBService.so.1.0.0)、数据库文件(ESSQLiteCE100.db)和平面图文件夹(PageBackgroundFolder)吗？";
        messageBox.setText(log);
        int ret = messageBox.exec();
        if(ret == QMessageBox::Cancel)
        {
    ...
    else if(ret == QMessageBox::No)
    {
        ...
        messageBox.button(QMessageBox::Cancel)->setText("取消");
        QString log;
        log = "确定U盘/root文件夹下包含程序文件(Monitor、libBService.so.1.0.0)吗？";
        messageBox.setText(log);
        int ret = messageBox.exec();
        if(ret == QMessageBox::Cancel)
        {
